...
make clean        # Clean build artifacts (includes compile_commands.json)
```

### 4. Build Artifacts
- `compile_commands.json` is auto-generated by `bear` during CI
- Added to `.gitignore` and `make clean` target
- Root version is tracked but core/ version should not be committed
