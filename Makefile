...
lint:
	...
	@DOCKER_BUILDKIT=1 docker compose run --rm -T dev bash -c "cd /workspace && ./tools/code-quality/lint.sh"

.PHONY: check-quality
check-quality:
	@echo "🔍 Running all quality checks (v2 - using standard tools)..."
	@DOCKER_BUILDKIT=1 docker compose run --rm -T dev bash -c "cd /workspace && chmod +x ./tools/code-quality/*.sh && ./tools/code-quality/run-all-checks-v2.sh"

.PHONY: check-quality-legacy
check-quality-legacy:
	@echo "🔍 Running legacy custom checks..."
	@DOCKER_BUILDKIT=1 docker compose run --rm -T dev bash -c "cd /workspace && ./tools/code-quality/run-all-checks.sh"

.PHONY: format
format:
	@echo "🎨 Formatting code with clang-format..."
	@DOCKER_BUILDKIT=1 docker compose run --rm -T dev bash -c "find /workspace/src -name '*.c' -o -name '*.h' | xargs clang-format -i"
	@echo "✅ Code formatted!"
