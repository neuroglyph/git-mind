...
void test_cbor_function_sizes(void) {
    const char *files[] = {"../src/attribution/cbor.c",
                           "../src/attribution/cbor_common.c",
                           "../src/attribution/cbor_decode_ex.c", NULL};
            ...
            if (in_function) {
                ...
                if (line[0] == '}' && strlen(line) < 3) {
                    if (line_count > MAX_FUNCTION_LINES) {
                        fprintf(
                            stderr,
                            "FAIL: %s: Function '%s' has %d lines (max %d)\n",
                            files[file_idx], func_name, line_count,
                            MAX_FUNCTION_LINES);
                        assert(0);
                    }
                    in_function = 0;
...
void test_cbor_uses_safe_memory(void) {
    ...
    const char *banned[] = {" memcpy(", " memset(",  " memmove(",
                            " strcpy(", " strncpy(", NULL};
    ...
    for (int file_idx = 0; files[file_idx]; file_idx++) {
        ...
        while (fgets(line, sizeof(line), fp)) {
            ...
            if (strstr(line, "for") && strstr(line, "int")) {
                /* Should NOT use _i_ (reserved) or i (too simple during
                 * migration) */
                if (strstr(line, "int _i_") || strstr(line, "int i ") ||
                    ...
                    fprintf(
                        stderr,
                        "      Use descriptive names like 'idx' or 'index'\n");
                    assert(0);
                }
            }
        }
