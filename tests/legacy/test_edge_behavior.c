...
void test_edge_create_success(void) {
    ...
    gm_edge_t edge;
    int result =
        gm_edge_create(&ctx, "src/a.c", "src/b.c", GM_REL_DEPENDS_ON, &edge);
...
void test_edge_path_boundaries(void) {
    gm_context_t ctx = {.time_ops = &test_time_ops};
    ...
    int result =
        gm_edge_create(&ctx, long_path, "short", GM_REL_DEPENDS_ON, &edge);
