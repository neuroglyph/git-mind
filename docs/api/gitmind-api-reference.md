...
1. **libgit2** - Production backend using libgit2
2. **test** - Mock backend for unit testing

## Best Practices

1. **Always check return values** - Every function can fail
2. **Free resources** - Use `gm_link_set_free()` and `gm_destroy_context()`
3. **Use filters wisely** - Filter at query time, not after
4. **Handle tombstones** - Unlinked items won't appear in lists
5. **Respect depth limits** - Traverse depth is capped at 10
6. **Use appropriate output modes** - Verbose for humans, porcelain for scripts
