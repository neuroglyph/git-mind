...
static void test_utf8_ascii(void) {
    const char *valid[] = {
        "hello", "Hello, World!", "1234567890",
        "!@#$%^&*()_+-=", "" /* Empty string is valid */
    };
...
static void test_utf8_overlong(void) {
    ...
    for (size_t i = 0; i < sizeof(overlong) / sizeof(overlong[0]); i++) {
        gm_utf8_error_t err =
            gm_utf8_validate(overlong[i].data, overlong[i].len);
        if (err == GM_UTF8_OK) {
...
static void test_utf8_truncated(void) {
    const struct {
        ...
        size_t len;
        const char *desc;
    } truncated[] = {/* Missing continuation bytes */
    ...
    for (size_t i = 0; i < sizeof(truncated) / sizeof(truncated[0]); i++) {
        gm_utf8_error_t err =
            gm_utf8_validate(truncated[i].data, truncated[i].len);
...
static void test_utf8_surrogates(void) {
    const struct {
        ...
        size_t len;
        const char *desc;
    } surrogates[] = {/* UTF-16 surrogates (D800-DFFF) are invalid in UTF-8 */
    ...
    for (size_t i = 0; i < sizeof(surrogates) / sizeof(surrogates[0]); i++) {
        gm_utf8_error_t err =
            gm_utf8_validate(surrogates[i].data, surrogates[i].len);
...
static void test_utf8_out_of_range(void) {
    const struct {
        ...
        size_t len;
        const char *desc;
    } out_of_range[] = {/* Beyond U+10FFFF */
                        {"\xF4\x90\x80\x80", 4, "U+110000"},
                        {"\xF4\x90\x80\x81", 4, "U+110001"},
                        {"\xF5\x80\x80\x80", 4, "U+140000"},
                        {"\xF7\xBF\xBF\xBF", 4, "U+1FFFFF"}};

    for (size_t i = 0; i < sizeof(out_of_range) / sizeof(out_of_range[0]);
         i++) {
        gm_utf8_error_t err =
            gm_utf8_validate(out_of_range[i].data, out_of_range[i].len);
        if (err != GM_UTF8_ERR_OUT_OF_RANGE &&
            err != GM_UTF8_ERR_INVALID_START) {
            printf("❌ Failed to reject out of range: %s (got %d)\n",
                   out_of_range[i].desc, err);
        }
        assert(err == GM_UTF8_ERR_OUT_OF_RANGE ||
               err == GM_UTF8_ERR_INVALID_START);
    }
...
static void test_utf8_streaming(void) {
    /* Test 1: Valid multi-chunk */
    {
        ...
        assert(gm_utf8_validate_chunk(&state, "\xE4\xB8\x96\xE7\x95\x8C", 6) ==
               GM_UTF8_OK);
