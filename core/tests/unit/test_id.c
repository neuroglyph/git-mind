...
static void test_id_from_string(void) {
    ...
    gm_result_id_t result1 = gm_id_from_string_with_context(&ctx, "hello");
    assert(GM_IS_OK(result1));
    gm_id_t id1 = GM_UNWRAP(result1);

    gm_result_id_t result2 = gm_id_from_string_with_context(&ctx, "hello");
    assert(GM_IS_OK(result2));
    gm_id_t id2 = GM_UNWRAP(result2);

    gm_result_id_t result3 = gm_id_from_string_with_context(&ctx, "world");
    ...
    gm_result_id_t err_result = gm_id_from_string_with_context(&ctx, nullptr);
...
static void test_id_hex_conversion(void) {
    ...
    gm_result_id_t id_result = gm_id_from_string_with_context(&ctx, "test");
...
static void test_typed_ids(void) {
    ...
    gm_result_id_t node1_result = gm_id_from_string_with_context(&ctx, "node1");
    assert(GM_IS_OK(node1_result));
    gm_node_id_t node1 = {.base = GM_UNWRAP(node1_result)};

    gm_result_id_t node2_result = gm_id_from_string_with_context(&ctx, "node2");
    assert(GM_IS_OK(node2_result));
    gm_node_id_t node2 = {.base = GM_UNWRAP(node2_result)};

    gm_result_id_t edge1_result = gm_id_from_string_with_context(&ctx, "edge1");
    assert(GM_IS_OK(edge1_result));
    gm_edge_id_t edge1 = {.base = GM_UNWRAP(edge1_result)};

    gm_result_id_t edge2_result = gm_id_from_string_with_context(&ctx, "edge2");
...
static void test_id_generate(void) {
    ...
    gm_result_id_t result1 = gm_id_generate_with_context(&ctx);
    assert(GM_IS_OK(result1));
    gm_id_t id1 = GM_UNWRAP(result1);

    gm_result_id_t result2 = gm_id_generate_with_context(&ctx);
    assert(GM_IS_OK(result2));
    gm_id_t id2 = GM_UNWRAP(result2);
...
static void test_session_id(void) {
    ...
    gm_result_session_id_t result1 = gm_session_id_new_with_context(&ctx);
    assert(GM_IS_OK(result1));
    gm_session_id_t sid1 = GM_UNWRAP(result1);

    gm_result_session_id_t result2 = gm_session_id_new_with_context(&ctx);
    assert(GM_IS_OK(result2));
...
static void test_id_from_data(void) {
    ...
    gm_result_id_t result1 = gm_id_from_data_with_context(&ctx, data1, sizeof(data1));
    assert(GM_IS_OK(result1));
    gm_id_t id1 = GM_UNWRAP(result1);

    gm_result_id_t result2 = gm_id_from_data_with_context(&ctx, data2, sizeof(data2));
    assert(GM_IS_OK(result2));
    gm_id_t id2 = GM_UNWRAP(result2);

    gm_result_id_t result3 = gm_id_from_data_with_context(&ctx, data3, sizeof(data3));
    assert(GM_IS_OK(result3));
    gm_id_t id3 = GM_UNWRAP(result3);
    ...
    gm_result_id_t err_result = gm_id_from_data_with_context(&ctx, nullptr, 10);
    assert(GM_IS_ERR(err_result));
    gm_error_free(GM_UNWRAP_ERR(err_result));

    /* Test zero length - should succeed (SHA256 of empty data is valid) */
    gm_result_id_t zero_result = gm_id_from_data_with_context(&ctx, data1, 0);
...
static void test_id_hash(void) {
    ...
    gm_result_u32_t hash1_result = gm_id_hash_with_context(&ctx, id1);
    assert(GM_IS_OK(hash1_result));
    uint32_t hash1 = GM_UNWRAP(hash1_result);

    gm_result_u32_t hash2_result = gm_id_hash_with_context(&ctx, id2);
    ...
    gm_result_u32_t hash3_result = gm_id_hash_with_context(&ctx, id3);
    assert(GM_IS_OK(hash3_result));
    uint32_t hash3 = GM_UNWRAP(hash3_result);

    gm_result_u32_t hash4_result = gm_id_hash_with_context(&ctx, id4);
    ...
    gm_result_u32_t rehash1 = gm_id_hash_with_context(&ctx, id1);
    assert(GM_IS_OK(rehash1) && GM_UNWRAP(rehash1) == hash1);

    gm_result_u32_t rehash2 = gm_id_hash_with_context(&ctx, id2);
