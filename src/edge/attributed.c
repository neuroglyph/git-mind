 ...
 */
int gm_edge_attributed_create(gm_context_t *ctx, const char *src_path,
                              const char *tgt_path, gm_rel_type_t rel_type,
                              uint16_t confidence,
                              const gm_attribution_t *attribution,
                              gm_lane_type_t lane, gm_edge_attributed_t *edge) {
    ...
    if (ctx && ctx->time_ops && ctx->time_ops->time) {
        edge->timestamp =
            (uint64_t)ctx->time_ops->time(NULL) * MILLIS_PER_SECOND;
    } else {
 ...
 */
int gm_edge_attributed_format(const gm_edge_attributed_t *edge, char *buffer,
                              size_t len) {
    ...
    snprintf(buffer, len, "%s ──%s──> %s", edge->src_path, rel_str,
             edge->tgt_path);
...
int gm_edge_attributed_format_with_attribution(const gm_edge_attributed_t *edge,
    ...
    int result =
        gm_edge_attributed_format(edge, basic_format, sizeof(basic_format));
    if (result != GM_OK) {
        return result;
    }
    ...
    const char *source_str =
        get_source_type_string(edge->attribution.source_type);
