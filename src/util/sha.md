...
1. **Predictable**: Only committed/staged content
2. **Fast**: In-memory structure
3. **Consistent**: Same view as `git status`
4. **Safe**: No dirty working tree surprises
...
1. **Correct**: Uses Git's exact SHA calculation
2. **Fast**: Direct index access, no I/O
3. **Simple**: ~40 lines of clear code
4. **Safe**: No working tree surprises
5. **Portable**: libgit2 handles platform quirks
