...
static void run_stampede(git_repository* repo, const char* branch, 
    ...
    for (int i = 0; i < wildebeest_count; i += 100) {
        int batch_size = (i + 100 < wildebeest_count) ? 100 : (wildebeest_count - i);
    ...
    printf("Found %zu edges in %.2f ms\n", scan_result.count, result->journal_scan_ms);
    ...
    printf("Found %zu edges in %.2f ms\n", cache_result.count, result->cache_query_ms);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 3; i++) {
        printf("%6d | %9.2f ms | %8.2f ms | %6.1fx\n",
               sizes[i], 
               results[i].journal_scan_ms,
               results[i].cache_query_ms,
               results[i].speedup);
    }
