...
- **Contract validation tests** — `test/contracts.test.js` (17 unit tests) validates schema compilation, envelope requirements, sample payloads, and optional field handling. `test/contracts.integration.test.js` (7 CLI canary tests) executes the real binary and validates output against schemas using `ajv` (#205)
...
- **Test count** — 366 tests across 22 files (was 342 across 20)
